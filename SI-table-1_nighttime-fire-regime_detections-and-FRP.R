...
# landcovers_of_interest <- c(18:20) # Just tundra
...
afd_frp_landcover <- lon_lat_landcover[index %in% landcovers_of_interest & detections_per_overpass != Inf, .(detections_per_overpass = sum(detections_per_overpass),
...
afd_frp_just_daynight <- lon_lat_landcover[index %in% landcovers_of_interest & detections_per_overpass != Inf, .(detections_per_overpass = sum(detections_per_overpass),
                                                                                                                 frp_per_overpass = sum(frp_per_overpass)),
                                           by = .(daynight)]
...
night_fire_regime_table <-
  ...
  dplyr::filter(index %in% landcovers_of_interest & index != 20) %>% 
