...
# thresholds ====================================================================
thresholds<-read_csv("data/case_studies/updated-goes-af-vpd-thresholds.csv")
# thresholds[1:20,]
...
df_colors <- read_csv("data/case_studies/landcover-colors-2021.csv") %>%
...
snowy_lc_file <- "data/case_studies/MCD12Q1_tifs/h29v12.tif"
tubbs_lc_file <- "data/case_studies/MCD12Q1_tifs/h08v05.tif"

modis_crs <- "+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +a=6371007.181 +b=6371007.181 +units=m +no_defs"

# snowy complex ================================================================

snowy <- st_read("data/case_studies/snowy_complex.gpkg") %>%
...
snowy_modis <- read_csv("data/case_studies/snowy_modis_afd.csv")%>%
...
snowy_clim <- read_csv("data/case_studies/snowyave.csv",col_names = F) %>%
...
if(!file.exists("data/case_studies/snowy_area.Rda")){
snowy_area <- st_read("data/case_studies/background/world_borders/ne_50m_admin_0_countries.shp") %>%
  filter(NAME_EN == "Australia") %>%
  st_crop(snowy %>% st_buffer(1))
  save(snowy_area, file = "data/case_studies/snowy_area.Rda")}else{
    load("data/case_studies/snowy_area.Rda")
  }
...
australia <- st_read("data/case_studies/background/world_borders/ne_50m_admin_0_countries.shp") %>%
  filter(NAME_EN == "Australia")


if(!file.exists("data/case_studies/fishnet.RDS")){
  fishnet <- st_bbox(snowy)%>% 
    ...
    exactextractr::exact_extract(x=raster(snowy_lc_file), y=., 
                   fun = "mode") 
  saveRDS(fishnet, "data/case_studies/fishnet.RDS")
  }else{
  fishnet <- readRDS("data/case_studies/fishnet.RDS")
}

if(!file.exists("data/case_studies/fishnet_lc.RDS")){
  fishnet_lc <- st_bbox(snowy) %>%
    ...
    mutate(lc= fishnet,
           classes = lut_lc[lc])
  saveRDS(fishnet_lc, "data/case_studies/fishnet_lc.RDS")
}else{
    fishnet_lc<-readRDS("data/case_studies/fishnet_lc.RDS")
  }
...
tubbs_viirs <- st_read("data/case_studies/tubbs.gpkg") %>%
  mutate(daynight = factor(daynight, levels = c("night", "day")))
daterange_t <- tubbs_viirs$acq_date %>% as.Date() %>% range()

tubbs_modis <- read_csv("data/case_studies/tubbs_modis_afd.csv")%>%
  dplyr::mutate(acq_hour = as.numeric(substr(acq_time, start = 1, stop = 2)),
                acq_min = as.numeric(substr(acq_time, start = 3, stop = 4)),
                acq_datetime = ymd_hm(paste0(acq_date, " ", acq_hour, ":", acq_min)),
                acq_year = year(acq_datetime),
...
tubbs_fired_p <- st_read("data/case_studies/tubbs_fired.gpkg")
...
tubbs_area <- st_read("data/case_studies/CUS/CUS.shp") %>%
  st_transform(crs=st_crs(tubbs_viirs)) %>%
  st_crop(tubbs_viirs %>% st_buffer(1))

usa <- st_read("data/case_studies/CUS/CUS.shp") %>%
  st_transform(crs=st_crs(tubbs_viirs)) %>%
  filter(STUSPS == "CA")
...
if(!file.exists("data/case_studies/fishnett.RDS")){
  fishnet_t <- st_bbox(tubbs_viirs)%>% 
    ...
    raster::extract(x=raster(tubbs_lc_file), y=., 
                    ...
                    method="simple") 
  saveRDS(fishnet, "data/case_studies/fishnett.RDS")
}else{
  fishnet_t <- readRDS("data/case_studies/fishnett.RDS")
}

if(!file.exists("data/case_studies/fishnet_lct.RDS")){
  fishnet_lc_t <- st_bbox(tubbs_viirs) %>%
    ...
    mutate(lc= fishnet,
           classes = lut_lc[lc])
  saveRDS(fishnet_lc_t, "data/case_studies/fishnet_lct.RDS")
}else{
  fishnet_lc_t<-readRDS("data/case_studies/fishnet_lct.RDS")
}


# snowy plots ==================================================================
if(!file.exists("data/case_studies/fishnet_snowwwy.Rda")){
lc_snowwwy <- fishnet_lc %>%
  group_by(classes) %>%
  summarise() %>%
  ungroup()
  save(lc_snowwwy, file="data/case_studies/fishnet_snowwwy.Rda")}else{
    load("data/case_studies/fishnet_snowwwy.Rda")}
...
if(!file.exists("data/case_studies/lc_tubbbs.Rda")){
  lc_tubbbs <- fishnet_lc_t %>%
    group_by(classes) %>%
    summarise() %>%
    ungroup()
  save(lc_tubbbs, file = "data/case_studies/lc_tubbbs.Rda")
  }else{load("data/case_studies/lc_tubbbs.Rda")}
...
inset_t_data <- read_csv("data/case_studies/tubbs-hourly.csv") %>% 
